...
#include "drivers/compass/compass_iis2mdc.h"
...
#elif defined(USE_MAG_HMC5883) || defined(USE_MAG_QMC5883) || defined(USE_MAG_AK8975) || defined(USE_MAG_IST8310) || defined(USE_MAG_IIS2MDC) || (defined(USE_MAG_AK8963) && !(defined(USE_GYRO_SPI_MPU6500) || defined(USE_GYRO_SPI_MPU9250)))
...
bool compassDetect(magDev_t *magDev, uint8_t *alignment)
{
    *alignment = ALIGN_DEFAULT;  // may be overridden if target specifies MAG_*_ALIGN
    ...
    case MAG_IIS2MDC:
        ...
        if (iis2mdcDetect(magDev)) {
#ifdef MAG_IIS2MDC_ALIGN
            *alignment = MAG_IIS2MDC_ALIGN;
#endif
            magHardware = MAG_IIS2MDC;
            break;
        }
#endif
        FALLTHROUGH;
